    ...
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 space-y-4">
            <h3 className="text-base font-bold text-white">Eligibility Criteria Evaluation</h3>

            <div className="space-y-3 text-xs">
              {selectedScheme.matchedRules && selectedScheme.matchedRules.length > 0 ? (
                selectedScheme.matchedRules.map((rule, idx) => (
                  <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-slate-800/50 border border-slate-700/50 text-xs">
                    <span className="text-slate-300 font-medium">{rule}</span>
                    <span className="font-semibold text-emerald-400 flex items-center gap-1 shrink-0">
                      <CheckCircle2 size={14} /> Passed
                    </span>
                  </div>
                ))
              ) : (
                <div className="p-3 rounded-xl bg-slate-800/50 text-slate-400 text-center">
                  Fill profile details to see evaluated criteria.
                </div>
              )}

              {selectedScheme.failedRules && selectedScheme.failedRules.map((fail, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-rose-950/20 border border-rose-900/40 text-xs">
                  <div className="space-y-0.5">
                    <span className="text-rose-200 font-bold">{typeof fail === 'string' ? fail : fail.rule}</span>
                    {typeof fail === 'object' && fail.actual && (
                      <p className="text-[11px] text-rose-400">{fail.actual} (Requirement: {fail.required})</p>
                    )}
                  </div>
                  <span className="font-semibold text-rose-400 shrink-0">Not Eligible</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <NearbyPartnersCard scheme={selectedScheme} showHeader={true} />
