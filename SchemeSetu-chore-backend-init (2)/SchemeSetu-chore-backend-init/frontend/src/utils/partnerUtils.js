...
export function getNearbyPartnersForScheme(partners = [], scheme = null, userProfile = {}) {
  ...
  const userPin = (userProfile?.pinCode || '').trim()

  return partners
    .filter((partner) => {
      if (!scheme) return true
      // Match by scheme name or ID or if partner supports all schemes
      if (!partner.supportedSchemes || partner.supportedSchemes.length === 0) return true
      return partner.supportedSchemes.some((s) => {
    ...
    .map((partner) => {
      ...
      let distanceKm = 3.5
      let distanceText = partner.distance || '3.5 km away'

      if (partner.lat && partner.lng && DISTRICT_COORDS[userDistrict]) {
        const userCoord = DISTRICT_COORDS[userDistrict]
        const km = calculateHaversineDistance(userCoord.lat, userCoord.lng, partner.lat, partner.lng)
