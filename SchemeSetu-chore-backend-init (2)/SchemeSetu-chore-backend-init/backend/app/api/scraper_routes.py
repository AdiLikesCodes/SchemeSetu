...
async def trigger_scrape(
    ...
    Flow:
    1. Quick Layer 1 domain check (synchronous, no DB).
    2. Create ScrapeJob with status=QUEUED.
    3. Return 202 + job_id immediately.
    4. Background task runs full pipeline.
    """
...
async def publish_scheme(scheme_id: str, request: PublishRequest) -> dict:
    ...
    Actions:
    1. Validates scheme is in a publishable state.
    2. Sets status → PUBLISHED.
    3. Creates an immutable SchemeVersion snapshot.
    4. Supersedes any previously published version of the same scheme.
    5. Appends audit log.

    ONLY PUBLISHED schemes are used by the deterministic eligibility engine.
    """
    scheme = await ScrapedScheme.find_one(ScrapedScheme.scheme_id == scheme_id)
    if not scheme:
        raise HTTPException(status_code=404, detail="Scheme not found.")
